...
public class NDetection extends Thread {
    ...
    public NDetection(List<Mat> originMat,CallBack callBack ,Handler handler) {
       ...
       // List<Integer>  touchAreas=new ArrayList<>();
        while (isRun){
            ...
            if (touchArea.size() > 0) {
                ...
                NTrigleEvent nTriglenow  = response(rects, touchArea, integers);
